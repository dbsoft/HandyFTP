...
then
    ...
    install_name_tool -change `otool -L handyftp | grep libdwindows | cut -d ' ' -f 1 | tr -d '\t'`  @executable_path/libdwindows.dylib "../install/package/$APPNAME.app/Contents/MacOS/$BINNAME"
    install_name_tool -change `otool -L handyftp | grep libdwcompat | cut -d ' ' -f 1 | tr -d '\t'` @executable_path/libdwcompat.dylib "../install/package/$APPNAME.app/Contents/MacOS/$BINNAME"
